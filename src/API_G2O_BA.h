...
/// g2o edge  观测值维度3 类型 Vector3d  优化节点第一个  VertexPose
class GNSSConstraint_sRt_Edge : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexPose> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  ...
  virtual void computeError() override {
        const VertexPose* v = static_cast<const VertexPose*>(_vertices[0]);
        Sophus::SE3d cam_pose = v->estimate(); // cw-ENU全局坐标
        //const  Eigen::Matrix3d R_cw = cam_pose.rotationMatrix(); 
        const  Eigen::Vector3d t_cw = cam_pose.translation();
        ...
        // Eigen::Vector4d t_cw_4(t_cw(0), t_cw(1), t_cw(2), 1);
        // Eigen::Vector4d t_cw_to_gw_4= _T_vo_to_GNSS_ENU*t_cw_4;
        // Eigen::Vector3d t_gw(t_cw_to_gw_4(0), t_cw_to_gw_4(1), t_cw_to_gw_4(2));

        Eigen::Vector3d t_gw = R_cw_to_gw*t_cw+t_cw_to_gw;// GNSS-ENU全局坐标
        // cam_pose.matrix() 
        //const  Eigen::Matrix3d R_cw = cam_pose.rotationMatrix(); 
        //const  Eigen::Vector3d t_wc = -R_cw.transpose() * t_cw;
        // 齐次t
        //const Vec4_t t_wc_h(t_wc(0), t_wc(1), t_wc(2), 1);
        //_error = srt * t_wc_h - _measurement; // slam->real ecef - 测量gps->ecef
        _error = t_gw - _measurement; //_measurement GNSS 真值  调用setmeasurement（） 设置  Vector3d
...
void bundleAdjustment_GNSS(
  ...
  for (size_t i = 0; i < N; i++) {
      ...
      // Edge
      GNSSConstraint_sRt_Edge *edge = new GNSSConstraint_sRt_Edge(T_vo_to_GNSS_ENU); //GNSS ENU真值 3D-2D是图像1的3d点
      edge->setId(i);
